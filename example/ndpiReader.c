...
static void parseOptions(int argc, char **argv) {
  ...
  while((opt = getopt_long(argc, argv, "c:df:g:i:hp:l:s:tv:V:n:j:rp:w:q0123:456:7:89:m:b:x:z:", longopts, &option_idx)) != EOF) {
#ifdef DEBUG_TRACE
...
static void node_proto_guess_walker(const void *node, ndpi_VISIT which, int depth, void *user_data) {
  ...
  if((which == ndpi_preorder) || (which == ndpi_leaf)) { /* Avoid walking the same node multiple times */    
   ...
   //ndpi_thread_info[thread_id].workflow->__flow_detected_callback(ndpi_thread_info[thread_id].workflow, flow, ndpi_thread_info[thread_id].workflow->__flow_detected_udata);
...
static void file_manipulation(struct ndpi_flow_info * flow)
{
  if (out_file_4 == NULL || out_file_6 == NULL) 
      {   
        printf("Error! Could not open file\n"); 
        exit(-1); // must include stdlib.h 
      } 
        
       if ((flow->src_name[0]=='4' && flow->src_name[2]=='2' && flow->src_name[4]=='2' && flow->src_name[6]=='2')||(flow->src_name[0]=='1' && flow->src_name[1]=='9' && flow->src_name[2]=='2')||(flow->src_name[0]=='1' && flow->src_name[1]=='6' && flow->src_name[2]=='3' && flow->src_name[4]=='1' && flow->src_name[5]=='2' && flow->src_name[6]=='1' && flow->src_name[8]=='1' && flow->src_name[9]=='2' && flow->src_name[10]=='8' && flow->src_name[12]=='1' && flow->src_name[13]=='3' && flow->src_name[14]=='5') || !strcmp(last_ip,flow->src_name));
        else 
        {
          
          if (flow->ip_version==4)
          fprintf(out_file_4,"%s\n",flow->src_name);
          else 
          fprintf(out_file_6,"%s\n",flow->src_name);
          strcpy(last_ip,flow->src_name);
        }
        if ((flow->dst_name[0]=='4' && flow->dst_name[2]=='2' && flow->dst_name[4]=='2' && flow->dst_name[6]=='2')||(flow->dst_name[0]=='1' && flow->dst_name[1]=='9' && flow->dst_name[2]=='2')||(flow->dst_name[0]=='1' && flow->dst_name[1]=='6' && flow->dst_name[2]=='3' && flow->dst_name[4]=='1' && flow->dst_name[5]=='2' && flow->dst_name[6]=='1' && flow->dst_name[8]=='1' && flow->dst_name[9]=='2' && flow->dst_name[10]=='8' && flow->dst_name[12]=='1' && flow->dst_name[13]=='3' && flow->dst_name[14]=='5') || !strcmp(last_ip,flow->dst_name));
        else
         {
...
static void on_protocol_discovered(struct ndpi_workflow * workflow,
				   ...
				   void * udata) 
{
 if (strcmp(ndpi_get_proto_name(workflow->ndpi_struct, flow->detected_protocol.app_protocol),target)==0)
 { 
  char delim[] = " ";
  char *ptr = strtok(target, delim);
  while (ptr != NULL)
  {
  if (strcmp(ndpi_get_proto_name(workflow->ndpi_struct, flow->detected_protocol.app_protocol),ptr)==0)
  {
      
  // test for files not existing. 
      file_manipulation(flow);
  }
...
#endif
    int i;    
    out_file_4 = fopen("/etc/dropv4.txt", "w"); // write only 
    out_file_6 = fopen("/etc/dropv6.txt", "w"); // write only
